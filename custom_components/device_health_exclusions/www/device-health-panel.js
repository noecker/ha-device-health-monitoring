 ...
 * Copyright 2017 Google LLC
 * SPDX-License-Identifier: BSD-3-Clause
 */function pt(t){return ht({...t,state:!0,attribute:!1})}const ut=n`
  :host {
    display: block;
    padding: 16px;
...
`;async function gt(t,e){return(await t.callWS({type:"device_health_exclusions/get_problem_devices",battery_threshold:e})).devices}async function _t(t){return await t.callWS({type:"device_health_exclusions/get_exclusions"})}let vt=class extends nt{constructor(){super(...arguments),this.filterMode="all",this.searchQuery="",this.batteryThreshold=20}render(){return Q`
      <div class="toolbar">
    ...
    `}_setFilter(t){this.filterMode=t,this.dispatchEvent(new CustomEvent("filter-changed",{detail:{filterMode:t}}))}_handleSearch(t){const e=t.target;this.searchQuery=e.value,this.dispatchEvent(new CustomEvent("search-changed",{detail:{searchQuery:this.searchQuery}}))}_handleThresholdChange(t){const e=t.target,s=parseInt(e.value,10);s>=5&&s<=50&&(this.batteryThreshold=s,this.dispatchEvent(new CustomEvent("threshold-changed",{detail:{batteryThreshold:s}})))}};vt.styles=n`
    :host {
      display: block;
    }
  ...
  `,t([ht({type:String})],vt.prototype,"filterMode",void 0),t([ht({type:String})],vt.prototype,"searchQuery",void 0),t([ht({type:Number})],vt.prototype,"batteryThreshold",void 0),vt=t([lt("filter-toolbar")],vt);let yt=class extends nt{constructor(){super(...arguments),this.devices=[],this.filterMode="all",this.searchQuery=""}render(){const t=this._getFilteredDevices();return 0===t.length?Q`
    ...
    `}_renderStatusBadge(t){return"unavailable"===t.state||"unknown"===t.state?Q`
      ...
      `:Q`<span>-</span>`}_getFilteredDevices(){let t=this.devices;if("excluded"===this.filterMode?t=t.filter(t=>t.is_excluded):"not_excluded"===this.filterMode&&(t=t.filter(t=>!t.is_excluded)),this.searchQuery){const e=this.searchQuery.toLowerCase();t=t.filter(t=>t.name.toLowerCase().includes(e)||t.entity_id.toLowerCase().includes(e))}return t}_toggleDevice(t){this.dispatchEvent(new CustomEvent("toggle-exclusion",{detail:{entityId:t},bubbles:!0,composed:!0}))}};yt.styles=n`
    :host {
      display: block;
    }
  ...
  `,t([ht({type:Array})],yt.prototype,"devices",void 0),t([ht({type:String})],yt.prototype,"filterMode",void 0),t([ht({type:String})],yt.prototype,"searchQuery",void 0),yt=t([lt("exclusions-table")],yt);let bt=class extends nt{constructor(){super(...arguments),this._devices=[],this._settings={excluded_entities:[],battery_threshold:20},this._filterMode="all",this._searchQuery="",this._loading=!0}connectedCallback(){super.connectedCallback(),this._loadData()}render(){if(this._loading)return Q`
        <div class="loading">
